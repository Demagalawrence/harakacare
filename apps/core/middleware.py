...
class EnsureTablesMiddleware(MiddlewareMixin):
    """
    Automatically ensure all required tables exist before processing requests
    """
        ...
        try:
            with connection.cursor() as cursor:
                ...
                cursor.execute("""
                    ...
                    WHERE type='table' AND name IN ('triage_villagecoordinates', 'triage_triagesession', 'triage_triagecase')
                """)
                existing_tables = [row[0] for row in cursor.fetchall()]
                
                required_tables = ['triage_villagecoordinates', 'triage_triagesession', 'triage_triagecase']
    ...
    def _create_missing_tables(self, cursor):
        """Create any missing triage tables"""
        ...
        # Create triage_triagesession table with all required columns
